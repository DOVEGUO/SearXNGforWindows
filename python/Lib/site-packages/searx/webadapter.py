...
def get_search_query_from_webapp(
    ...
    engine locale is the neutral ``all`` so engines can follow the query
    language instead of the browser UI language. The *selected locale*
    reported to the UI remains ``auto``.
    ...
    if query_lang == 'auto':
        # Neutral locale lets Google CSE / Bing infer language from the query
        # instead of inheriting the browser UI language (e.g. zh-CN), which
        # skews metasearch ranking away from google.com / bing.com.
        query_lang = 'all'
