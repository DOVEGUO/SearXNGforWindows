...
paging = True
"""Enable result pagination via Bing's ``first`` offset."""
safesearch = True
...
"""Offset step used for the ``first`` query argument."""
...
def request(query: str, params: "OnlineParams"):
    ...
    # Without an explicit market Bing follows the outbound proxy egress
    # locale (often JP/US) and returns unrelated regional noise for Chinese
    # queries when SearXNG locale is the neutral ``all``.
    if not engine_region or engine_region == "clear":
        engine_region = "zh-CN"
    ...
    # Page 1 must omit ``first``; later pages use 11, 21, ... Bing's HTML
    # scraper path does not depend on the JS infinite-scroll endpoints.
    pageno = int(params.get("pageno", 1) or 1)
    if pageno > 1:
        query_params["first"] = (pageno - 1) * _results_per_page + 1
